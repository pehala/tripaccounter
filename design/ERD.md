...
net_micro[p]  = paid_micro[p] − owed_micro[p]
```
One `GROUP BY` over the view per currency. `net > 0` → is owed money. `net < 0` →
owes. `|Σ net_micro| ≤ number of shares` per currency (floor loss), not exactly zero.
...
3. Greedy min-cash-flow on `net_cents`: repeatedly match the largest creditor with the
   largest debtor. At most n−1 transfers per currency, in hundredths, summing to zero. Settling happens after the
trip at a rate the humans agree on; the app only says "Bob → Ann: 4 200 ISK". There
is deliberately **no settlement table**: paybacks are not recorded, the trip is settled
once at the end from the suggestions. If mid-trip paybacks ever matter, a
...
- Person → blocked if referenced by an item/share; offer `active = false`
  instead, which hides them from new-item forms but keeps history intact.
- Currency → blocked if referenced.
