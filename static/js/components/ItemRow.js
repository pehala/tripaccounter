  ...
  return html`
    <a href="#" class="list-group-item list-group-item-action d-flex gap-3"
      ...
      <span class="flex-grow-1 min-w-0">
        ...
        <small class="text-body-secondary d-block item-line">${country ? `${country.flag} ${country.name} · ` : ''}${time} · <${Avatar} person=${payer} /> ${t('items.paid_by', { name: payer?.name || '' })}${hasMap ? html` · <i class="bi bi-geo-alt-fill text-primary"></i> ${mapLabel}` : ''}${item.labels.length > 0 ? html` · ${item.labels.map((name) => html`<${LabelBadge} key=${name} name=${name} />`)}` : ''}</small>
