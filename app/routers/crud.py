...
class TripChildRoutes:
    ...
    def delete(cls, session, row) -> None:
        """Delete one row."""
        run_field("id", cls.delete_row, session, row)
    ...
    def router(cls) -> APIRouter:
        ...
        def list_rows(**kw):
            session = kw["session"]
            rows = cls.rows(kw["trip"], session)
            return {cls.collection: [cls.serialize(row, session) for row in rows]}

        def create_row(**kw):
            session = kw["session"]
            row = run_field(cls.conflict_field, cls.create, session, kw["trip"], kw["body"])
            return {cls.resource: cls.serialize(row, session)}

        def update_row(**kw):
            session = kw["session"]
            row = cls.fetch(session, kw["trip"], kw[id_name])
            row = run_field(cls.conflict_field, cls.update, session, row, kw["body"])
            return {cls.resource: cls.serialize(row, session)}
